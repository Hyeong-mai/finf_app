...
class TimeBaseRecordPage extends StatefulWidget {
  const TimeBaseRecordPage({super.key});

  @override
  State<TimeBaseRecordPage> createState() => _TimeBaseRecordPageState();
}

class _TimeBaseRecordPageState extends State<TimeBaseRecordPage> {
  bool _showTimer = false;
  late TimeBasedTimerProvider timerProvider;
  late SettingsController settingsController;

  @override
  void initState() {
    super.initState();
    timerProvider = Get.find<TimeBasedTimerProvider>();
                    ...
                    children: [
                      ...
                      SingleChildScrollView(
                        child: Column(
                          children: [
                            ...List.generate(
                              settingsController.timeBasedRounds.value,
                              (index) {
                                final round = index + 1;
                                final isFirstRound = round == 1;
                                final skipFirstPrep = settingsController
                                    .timeBasedSkipFirstPrep
                                    .value;
                                final prepTime = (120 - (round - 1) * 15).clamp(
                                  30,
                                  120,
                                );
                                final staticTime = settingsController
                                    .getStaticRecordFinalSeconds();
                                final totalRoundTime =
                                    (isFirstRound && skipFirstPrep)
                                    ? staticTime
                                    : (prepTime + staticTime);
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      if (isFirstRound && skipFirstPrep)
                                        ...
                                        Expanded(
                                          child: Text(
                                            '${TimerUtils.formatTime(prepTime)}',
                                            textAlign: TextAlign.center,
                                            style: AppTextStyles.b1r('white'),
                                          ),
                                        ),
  ...
  Widget _buildTimerPage() {
    return Container(
      ...
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 24),
      child: GetBuilder<TimeBasedTimerProvider>(
        builder: (controller) {
          return CommonTimer(
